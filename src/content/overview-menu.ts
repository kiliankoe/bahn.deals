...
const qs = (root: Document | Element, sel: string) => root.querySelector(sel) as HTMLElement | null;
const qsa = (root: Document | Element, sel: string) => Array.from(root.querySelectorAll(sel)) as HTMLElement[];

const findConnectionRoot = (el: Element | null): HTMLElement | null => {
  let cur: any = el;
...
const extractSelection = (root: HTMLElement) => {
...
const buildMenuItem = () => {
  ...
  btn.style.cssText = `display:flex;align-items:center;width:100%;padding:0.75rem 1rem;background:transparent;border:none;text-align:left;cursor:pointer;transition:background-color .2s ease;font-family:inherit;font-size:1rem;color:#282d37;border-radius:4px;margin:0;`;

  btn.addEventListener('mouseenter', () => (btn.style.backgroundColor = '#f0f3f5'));
  btn.addEventListener('mouseleave', () => (btn.style.backgroundColor = 'transparent'));

  const content = document.createElement('span');
  content.className = 'db-web-button__content';
  content.style.cssText = 'display:flex;align-items:center;gap:.75rem;width:100%';
  ...
  icon.innerHTML = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="8.5" cy="8.5" r="5.75" stroke="#EC0016" stroke-width="1.5"/><path d="M12.75 12.75L17 17" stroke="#EC0016" stroke-width="1.5" stroke-linecap="round"/></svg>`;
...
const injectIntoMenu = (menuRoot: HTMLElement, connectionRoot: HTMLElement) => {
  ...
  btn.addEventListener('click', async (ev) => {
    ...
    try {
      await browser.runtime.sendMessage({ type: BD_MSG.OPEN_ANALYSIS, selection });
    } catch {}
  });
  ul.appendChild(li);
  return true;
...
const observeMenuOpen = (actionMenuEl: HTMLElement, connectionRoot: HTMLElement) => {
  const mo = new MutationObserver((mutations, obs) => {
    for (const m of mutations) {
      for (const n of Array.from(m.addedNodes)) {
        if (!(n instanceof HTMLElement)) continue;
        if (n.matches?.('ul[role="menu"], div._content, div[role="menu"]')) injectIntoMenu(actionMenuEl, connectionRoot);
      }
    }
  });
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (!target) return;
    const menuButton = target.closest?.('.test-menu-button, .DBWebActionMenu .db-web-button--type-icon') as HTMLElement | null;
    if (!menuButton) return;
    const connectionRoot = findConnectionRoot(menuButton);
    if (!connectionRoot) return;
    const actionMenu = menuButton.closest?.('.DBWebActionMenu') as HTMLElement | null;
