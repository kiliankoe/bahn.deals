...
const rnd = (n=16) => Array.from(crypto.getRandomValues(new Uint8Array(n)), b => b.toString(16).padStart(2, '0')).join('');
...
const formatOffset = (d) => {
  ...
  const sign = off >= 0 ? '+' : '-';
  const a = Math.floor(Math.abs(off) / 60).toString().padStart(2, '0');
...
async function dbnavRefreshTickets({ refreshToken, opts }) {
  ...
  try { console.debug('[dbnav] recon body', JSON.stringify(body).slice(0, 500)); } catch { console.debug('[dbnav] recon body (object)', body); }
...
async function dbnavJourneys({ fromEva, toEva, depDateTime, opts }) {
  ...
  try { console.debug('[dbnav] journeys body', JSON.stringify(body)); } catch { console.debug('[dbnav] journeys body (object)', body); }
